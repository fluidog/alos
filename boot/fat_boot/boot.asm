...
%endif
	...
	jmp short __start
	nop
	BS_OEMName 		DB	'MakeByLq'
	BPB_BytsPerSec 	Dw	512
	BPB_SecPerClus	DB	4				;每簇扇区数
	BPB_RsvdSecCnt	Dw	1				
	BPB_NumFATs		DB	2				
	BPB_RootEntCnt	Dw	512
	BPB_TotSec16	DW	0xfff0			;扇区数(16位)
	BPB_Media		DB	0xf8
	BPB_SecPerFat	DW	64				;每FAT扇区数
	BPB_SecPerTrk	DW	63				;每磁道扇区数
	BPB_NumHeads	DW	16			;磁头数
	BPB_HiddSec		DD	0				
	BPB_TotSec32	DD	0				;扇区数(32位)
	
	BS_DrvNum		DB	0x80				
	BS_Reserved		DB	0				;保留扇区		
	BS_BootSig		DB	0x29			
	BS_VolID		DD	0x08b0bb4d		
...
Loadfile:
	...
	mov ax,[BPB_SecPerFat]		;每FAT扇区数
	add ax,ax	
	add ax,[BPB_RsvdSecCnt]		;保留扇区数

	mov cl,1
	call ReadSector				;读入目录项
