...
s8 init_fat16(void)
{
	...
	if(!(boot=(boot_sector *)malloc(512))) 
		return -1;
	if(ide_read(boot,0,1)<0) 	//读取boot扇区
		return -1;
...
static s8 get_dir(u32 lba) 	//如果目录项扇区不连续，则会出错,小心子目录
{
	...
	if(!(entry=(dir_entry *)malloc(dirSize)))
			return -1;	
	while(1){
		if(ide_read((void*)((u32)entry+dirSize-512),lba++,1)<0)
			return -1;
...
static s8 get_fat(u16 clust)//默认读一个扇区fat
{
	if( (clust<fat16.cur_fat.start) || \
		...
		if(ide_read(fat16.cur_fat.clust,fat16.fat_start+clust/256,1)<0)
			return -1;
	}
	return 0;
}
...
s8 file_fat_read(const char *fileName, void *buffer, u32 start,u32 size)
{
	...
	while(size){
		...
		if(ide_read(buf,CLU2SEC(clust),fat16.clust_size)<0)	//read data
			return -1;
