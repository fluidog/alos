...
#include<string.h>
#include<fat.h>
#define DEBUG(format, args...) printk(const char * fmt, ...)
typedef u32 pte_t; 
typedef u32 pde_t; 
...
void page_fault(int errorcode,u32 ip,u16 seg)
{
	...
	DEBUG("err:%d fault_ip:0x%x ret_seg:0x%x ip:0x%x\n",errorcode,virAddr,seg,ip);
...
{
	pde_t * pdeHander;
	if((pdeHander=(pde_t *)(get_free_page()+VM_START))==(pde_t*)VM_START)
		return -1;
...
int copy_page_table(task_struct *newTask,u32 startAddr,u32 size)
{
	...
	for(;startAddr<endAddr;startAddr+=PAGE_SIZE){
		...
		if(put_page(pdeHander,startAddr,phyAddr,USER_PT_ATTR)<0)
			return -1;
	}
}
