...
{	
	...
	DEBUG("pagefault:%d pid:%d fault_ip:0x%x ret_ip:0x%x:0x%x\n",errorcode,current->pid,err_virAddr,seg,ip);
...
static void do_wp_page(u32 err_virAddr)
{
	...
	//DEBUG("%d do_wp_page:viraddr:0x%x oldPage:0x%x newPage:0x%x\n",\
		//current->pid,err_virAddr,oldPage,pte[(err_virAddr>>12)&0x3ff]);
...
/*释放当前进程所占用的用户空间的物理页,并设置其页表,注:内核空间共享所以无需释放，
并且，也不修改内核空间的页表，使其依然能访问内核空间,此函数仅被exit和execve函数调用*/
void free_page_table()
{
	...
	pde=(u32 *)(current->cr3+VM_START);
	for(i=0;i<VM_START>>22;i++){
		if(!(pde[i]&0x1))//不存在相应页表
			continue;

		//释放页表上有效的项
		pte = (u32 *)((pde[i] & 0xfffff000)+VM_START);	
		for(n=0;n<1024;n++){
			if(!(pte[n]&0x1))
				continue ;
			//存在对应物理页
			mem_map[ADDR2MAP(pte[n])]--;//低12位对页号无影响
			//pte[n]=0;之后页表将无效
		}
		
		//修改相应页目录项，并释放页表占用的物理页
		mem_map[ADDR2MAP(pde[i])]--;
		pde[i]=0;//页表无效
	}
