...
typedef struct d_inode {
     ...
     __le32     i_blocks;     /* 文件的数据块数 *///指物理扇区数..很奇怪
     __le32     i_flags;     /* 文件标志 */
...
/* these are in memory also */
	...
	unsigned int i_dev;		// i 节点所在的设备号。
	unsigned int i_num;		// i 节点号。
	unsigned int i_count;	// i 节点被使用的次数，0 表示该i 节点空闲。
	unsigned int i_lock;		// 锁定标志。
	unsigned int i_dirt;		// 已修改(脏)标志。
	unsigned int i_pipe;		// 管道标志。
	unsigned int i_mount;	// 安装标志。
	unsigned int i_seek;		// 搜寻标志(lseek 时)。
	unsigned int i_update;	// 更新标志。暂时无用
}m_inode;


#define EXT2_ENTRY_UNKNOW	0
#define EXT2_ENTRY_REG		1
#define EXT2_ENTRY_DIR		2
#define EXT2_ENTRY_CHR		3
#define EXT2_ENTRY_BLK		4
#define EXT2_ENTRY_FIFO		5
#define EXT2_ENTRY_SOCKET	6
#define EXT2_ENTRY_LNK		7
...
typedef struct buffer_head
{
  ...
  unsigned int b_blocknr;	/* block number */// 块号。
...
typedef struct file
{
  unsigned int f_mode;	// 文件操作模式（RW 位）
  unsigned int f_flags;	// 文件打开和控制的标志。
  unsigned int f_count;	// 对应文件句柄（文件描述符）数。
...
// 读取指定的数据块。
buffer_head * getblk(int dev,int block);//不一定更新,需检验update
buffer_head *bread (int dev, int block);//一定有效
buffer_head *breada (int dev, int block, ...);//预读
...
m_super_block * get_super(int dev);//只能得到已mount设备
