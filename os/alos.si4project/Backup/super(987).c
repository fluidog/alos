...
int ROOT_DEV;
buffer_head* get_group(m_super_block *sb,int g_num,group_desc **gd)
{
	buffer_head *bh;
	int block,groups;
	struct group_desc *g;
	...
	block=g_num/GROUPS_PER_BLOCK+2;
	if(!(bh=bread(sb->s_dev,block)))
		return NULL;
	
	*gd=(group_desc *)bh->b_data + g_num%GROUPS_PER_BLOCK;
	return bh;
}
