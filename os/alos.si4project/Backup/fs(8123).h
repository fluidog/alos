...
typedef struct m_super_block 
  ...
  buffer_head *s_group[8];	// 最多表示256组
  buffer_head *s_imap[256];	// i 节点位图缓冲块指针数组(占用8 块，可表示64M)。
  buffer_head *s_zmap[256];	// 逻辑块位图缓冲块指针数组（占用8 块）。
  unsigned short s_dev;		// 超级块所在的设备号。
  m_inode *s_isup;	// 被安装的文件系统根目录的i 节点。(isup-super i)
  m_inode *s_imount;	// 被安装到的i 节点。
...
typedef struct buffer_head
{
  ...
  buffer_head *b_prev;	// hash 队列上前一块（这四个指针用于缓冲区的管理）。
  buffer_head *b_next;	// hash 队列上下一块。
  buffer_head *b_prev_free;	// 空闲表上前一块。
  buffer_head *b_next_free;	// 空闲表上下一块。
}buffer_head;
