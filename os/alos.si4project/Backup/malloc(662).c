...
void* malloc(u32 len)
{
	...
	u16 blockNum=(len-1)/MALLOC_BLOCK_SIZE+1;
	...
	for(count=0,index=0;index < BLOCK_NUM;)
	{
		...
		else
		{
			...
			if(count>=blockNum)//找到符合要求的内存块
			{
				...
				BLOCK[index-count].mode |= ((0x10-len%MALLOC_BLOCK_SIZE)&0xf); //free size
...
void *realloc(void *addr, unsigned int newSize)
{
	...
	u32 oldSize=BLOCK[VA2BN(addr)].blockNum? \
				BLOCK[VA2BN(addr)].blockNum*MALLOC_BLOCK_SIZE:	\
				0x100*MALLOC_BLOCK_SIZE;
	int i;
